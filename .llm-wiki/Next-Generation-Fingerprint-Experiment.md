...
The common `nextgen_artifacts.py` boundary makes every future F09–F12 builder use checked canonical feature-family writes and revalidates persisted rows before matchup, SHAP, or model fitting. Dynamic rows require latest contributing game completion UTC < feature availability UTC < target kickoff UTC, distinct source and target IDs, and regular source history. Static Week-0 rows require null source-game fields, a documented pregame availability timestamp, and a target kickoff. The guard rejects 2026, postseason numeric history, target outcomes, market inputs, and pregame win probability. Boundary tests pass; actual F09–F12 builders and model runners do not yet exist. Empty 200 responses and HTTP 400/401/403/404 remain `needs_review` and block a stage.

`/plays/stats` exposes player-to-play stat attribution, not complete snap participation. It is **tentatively budgeted** as a full Stage E game-partition scenario for richer player-attributed F10/F12 research, without assuming an absence of PlayStat rows proves nonparticipation. On the provisional 2012–2025 pre-Stage-A schedule, it adds **11,509 first attempts**, bringing the combined plan to **13,265 first attempts**; after four earlier reservations, **6,731 local attempt slots** remain under the 20,000 cap for retries, quota checks, and subdivisions. The **projected, untested** combined footprint is **89,771,158,608 bytes** against the 100 GiB soft limit. Before full Stage E, sample stat meaning, row coverage, and 2,000-row response-cap handling; then recompute game partitions from fresh schedules and check live quota. No Stage E calls ran in preparation.
