...
2010–2023 is base development, 2024 is validation/design evidence, 2025 is late-development/retrospective evidence, and **2026 is untouched prospective evidence**. All feature discovery, equations, fitted weights, scaling, missingness, correlation, pruning, SHAP, hyperparameters, lineage, recommendation, and design evaluation must operate on seasons through 2025. A 2026 cache may exist, but it is quarantined. Later workers must call `assert_design_years` before every design operation and record the maximum season actually used. Do not describe 2025 as untouched for this extension.
...
**CFBD experiment call ceiling: 20,000 total outbound attempts**, including the one-call key check and every retry. This is below the account's stated 30,000-call allowance. All next-generation CFBD clients must use the shared persistent ledger at `results/cfbd_api_call_budget.json` under the artifact root and reserve a call before each HTTP attempt. The budget fails closed at the limit or if its ledger disappears. The ignored repository `.env` holds `CFBD_API_KEY` with owner-only permissions; `configs/fetch/nextgen_single_year.yaml` loads it without printing the value. `python3 scripts/check_nextgen_cfbd_key.py` spends exactly one budgeted request and makes no retry. Do not run that check repeatedly.
...
Diagnostic selection is capped at the top 1,000 advanced features by consensus normalized SHAP. Each selected feature gets a PNG and Markdown explanation under `feature_diagnostics/` with a lightweight index. Plot a pre-target-game feature against **next-game margin** by default and next-game empirical win frequency or other future outcomes when useful. Spearman is primary; show Pearson where helpful. The rushing Q4-minus-Q1 pair gets a polished next-game margin/win-frequency diagnostic in the later launch. Diagnostic correlation is not the pruning criterion.
...
Reuse `cfbd_fetch_v2.py`, `configs/features/feature_registry.yaml`, `configs/features/feature_ladders.yaml`, `MatchupBuilder` and reviewed unit pairings, `publication/scientific_shap_study.py`, the F6 compression study as evidence, SGE array and atomic Parquet fragment patterns. The current CFBD fetcher is not ready for a complete next-generation pull: it uses season-wide assumptions for some endpoints, silently treats some HTTP 400 responses as empty, defaults to more than three retry attempts, and can cache partial weekly results. Extend and validate it before launch; do not route the new program through the frozen confirmatory config.
