...
2010–2023 is base development, 2024 is design-informed internal validation, 2025 is design-informed late development, and **2026 is untouched prospective evidence**. Because 2024 and 2025 can influence design and lineage selection, their reported metrics are development metrics, not unbiased holdout estimates. All feature discovery, equations, fitted weights, scaling, missingness, correlation, pruning, SHAP, hyperparameters, lineage, recommendation, and design evaluation must operate on seasons through 2025. A 2026 cache may exist, but it is quarantined. Later workers must call `assert_design_operation_frame` at each design operation's input boundary and record the maximum season actually used. Do not describe 2025 as untouched for this extension.

Future F09–F12 feature builders must call `assert_temporal_feature_rows` before a feature frame reaches fitting or ranking, with source and target game IDs and timestamps. The guard rejects missing provenance, same-game source rows, late availability, postseason rows, quarantined seasons, outcome columns, and forbidden feature names. Its synthetic preflight check passes; end-to-end enforcement awaits those builders.
...
For each generation/design, recommend one F/LR/PR lineage (F/R for F06). Report 2024 and 2025 separately as design-influenced development metrics; do not claim an unbiased holdout estimate from them. Within 0.5 MAE points of best, break ties by Brier, ATS, upset, chalk, and feature count. Retain every run, including failures, in one ultra-wide Parquet with run and fingerprint×architecture summary rows. Discard checkpoints and routine logs after extracting compact diagnostics.

## Operations and artifacts

Large artifacts live under `/groups/bsavoie2/tburton2/TDNet/fingerprint_nextgen/`, with a ~100 GB soft budget. Check group free space and delete temporary SHAP/scratch/log data. The acquisition plan is in `nextgen_acquisition_v1.json`; existing CFBD partitions are reused only after request-scope and coverage proof. `/games` remains the local schedule seed and `/games/teams` is checked against that seed; this does not prove provider-side full-season completeness. Other legacy year files remain refetch or review candidates. Empty 200 responses and HTTP 400/401/403/404 require review and block stage progress, rather than proving structural unavailability. A schema/content incompatibility gets a new cache version. Do not cache a response that hits an endpoint cap as complete. Use at most three request attempts, polite backoff, and record unresolved gaps.
