...
Use `configs/publication/confirmatory_protocol.yaml` as the machine-readable source of truth for the 2026 confirmatory design and `docs/publication_2026/CONFIRMATORY_PROTOCOL.md` for its publication-facing interpretation. The README is the current high-level project map. `docs/TDNET_MASTER_PLAN.md` contains valuable design history but also older protocol language; its superseded claims are summarized in [TDNet Master Plan Source](Source-TDNet-Master-Plan).
