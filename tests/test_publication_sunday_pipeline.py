...
def test_sunday_pipeline_requires_certified_snapshot_and_writes_review_bundle(tmp_path: Path):
    ...
    assert (output / "figures/sunday_performance.png").exists()
    assert (output / "tables/scorecard.csv").exists()
    assert (output / "blog/post.txt").exists()
    assert not (output / "x_post_package").exists()
    assert (bundle / "public/predictions.parquet").exists()


def test_sunday_pipeline_accepts_minimal_postgame_results_certificate(tmp_path: Path):
    project = Path(__file__).resolve().parents[1]
    now = datetime.now(timezone.utc)
    predictions = pd.DataFrame([{
        "season": 2026,
    ...
    pd.DataFrame([{"game_id": 43, "home_points": 24, "away_points": 17}]).to_csv(
        results, index=False
    )
    snapshot = tmp_path / "snapshot.json"
    snapshot.write_text(
        json.dumps({"status": "pass", "certification": "postgame_results_certified"})
        + "\n"
    )
    output = tmp_path / "publication/2026/week_01/post_game"
    env = {
    ...
    subprocess.run(
        [
            sys.executable,
            "src/gridiron_ml/cli/publication/run_sunday_publication_pipeline.py",
            "--bundle",
            str(bundle),
            "--results",
            str(results),
